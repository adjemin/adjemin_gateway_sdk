...
class _OperatorPickerWidgetState extends State<OperatorPickerWidget> {
  ...
  final bool _isWaitingAcceptation = true;
  GatewayOperator? _paymentOperatorSelected;
  Timer? _transactionCheckTimer;

  StreamController<PaymentEvent> paymentStreamController =
            ...
            if (event.data is GatewayException) {
              ...
              if (mounted) {
                if (event.data.code == 300 &&
                    event.data.error!.contains('(203)')) {
                  displayErrorMessage(context,
                      "Une transaction similaire a été effectuée récemment, veuillez patienter 15 mins avant de réessayer",
                      () {
                  ...
                  if (errorMessage == "Payment has failed") {
                    ...
                    errorMessage =
                        event.data.error ?? "Une erreur est survenue";
                  }
          ...
          : Scaffold(
              ...
              appBar: AppBar(
                ...
                actions: [
                  GestureDetector(
                    onTap: () {
                      displayPrompt(context, "Êtes-vous sûr de faire cela?",
                          "Voulez vous confirmer que vous quittez sans finaliser le paiement ?",
                          () {
  ...
  void loadData() {
    ...
    GatewayRepository()
        .findOperatorsByCountry(
            widget.baseUrl, widget.countryCode.toString().split('.').last)
        .then((value) {
      hideProgress();
      if (mounted) {
        setState(() {
          elements =
              value.where((element) => element.isActivePayin == true).toList();
            ...
            children: elements
                .map((e) => GestureDetector(
                      onTap: () async {
                        ...
                        Customer? mCustomer;
                        print(
                            "Operator => ${e.name!.toLowerCase().contains('carte bancaire')}");
                        if (e.name!.toLowerCase().contains('carte')) {
                          mCustomer = widget.customer;
                        } else {
                          mCustomer = await Navigator.push(
                              context,
                              MaterialPageRoute(
  ...
  void _createPayment() {
    GatewayRepository()
      ...
      if (onError is SocketException) {
        ...
        displayErrorMessage(
            context, "Désolé, nous rencontrons des problèmes, revenez plutard.",
            () {
          Navigator.of(context).pop();
        });
      }
  ...
  void _pay(
      ...
      required GatewayOperator gatewayOperator,
      String? otp}) {
  ...
  Widget _buildPaymentLoadingUi() {
    return _paymentOperatorSelected == null
        ? Container(
            color: Colors.white,
          )
        : Scaffold(
            body: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Center(
                    child: Image.network(
                      _paymentOperatorSelected!.image!,
                      height: 80,
                    ),
                  ),
                  const SizedBox(
                  ...
                  Container(
                    ...
                    child: Text(
                      getLoadingMessage(widget.isPayIn,
                          _paymentOperatorSelected!, _isWaitingAcceptation),
  ...
  _processPayIn(GatewayOperator e, Customer mCustomer) async {
    if (e.name!.toLowerCase().contains('orange')) {
      final String? code = await Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) =>
                  OtpFormWidget(gatewayOperator: e, title: widget.title)));

      if (code != null && code.isNotEmpty) {
        final bool? hasPaymentResult = await Navigator.push(
            context,
            MaterialPageRoute(
  ...
  String getLoadingMessage(
      bool isPayIn, GatewayOperator operator, bool isWaitingAcceptation) {
    if (isPayIn) {
    ...
    GatewayRepository()
      ...
      if (value.status == GatewayTransaction.SUCCESSFUL) {
        paymentStreamController.add(PaymentEvent(
            currentState: PaymentState.COMPLETED, success: true, data: value));
      } else if (value.status == GatewayTransaction.FAILED) {
        paymentStreamController.add(PaymentEvent(
            currentState: PaymentState.COMPLETED, success: false, data: value));
      } else {
        paymentStreamController.add(PaymentEvent(
            currentState: PaymentState.PENDING, success: false, data: value));
      }
    }).catchError((onError) {
      paymentStreamController.add(PaymentEvent(
          currentState: PaymentState.COMPLETED, success: false, data: onError));
    });
  }
  ...
  displayPrompt(BuildContext context, String title, String message,
  ...
  // change launch instead to launchUrl because is deprecated
